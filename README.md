...
<!--  and Maven.   The new release should be up on Maven shortly (there is a period when the shield shows the new release number, but it is not yet up on Maven).  The Maven shield is below.
...
- Client script using `ws:` working whether or not `OPT` port (with value `"TLS"`) is connected to `WebSocketReceive` component - for both Firefox and Chrome browsers
<!-- 
